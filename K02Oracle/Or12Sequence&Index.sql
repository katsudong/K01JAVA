...
-시퀀스는 테이블 생성 후 별도로 만들어야한다. 즉 시퀀스는 테이블과
    독립적으로 저장되고 생성된다.
...
create sequence 시퀀스명
    ...
    [Cycle | NoCycle]   -> 최대/최소값에 도달할 경우 처음부터 다시
                          ...
                          증가 후 다시 시작값부터 재시작됨)
    [Cache | NoCache]   -> cache 메모리에 오라클서버가 시퀀스값을 할당하는가
                          여부를 지정

주의사항
1. start with 에 minvalue보다 작은값을 지정할 수 없다. 즉 start with값은
minvalue와 같거나 커야한다.
2. nocycle 로 설정하고 시퀀스를 계속 얻어올 때 maxvalue에 지정값을 초과하면
에러가 발생한다.
3. primary key에 cycle 옵션은 절대 지정하면 안된다.
*/

--제품정보를 입력할 테이블 생성
create table tb_goods (
    g_idx number(18) primary key,   /*상품의 일련번호*/
...
create sequence seq_serial_num
    ...
    cycle                   /*최대값 도달 시 시작값부터 재시작할 지 여부 : yes*/
    nocache;                /*캐시사용여부 : no*/

/*
    시퀀스 생성 후 최초 실행 시에는 오류가 발생된다.
...
insert into tb_goods values (seq_serial_num.nextval, '초콜렛13');/*
            ...
            사용할 시퀀스는 cycle 옵션을 사용하지 않아야 한다.
    */
...
start with 는 수정할 수 없다. 이미 생성된 시퀀스의 초기값은 수정할 수 없다.
*/
...
--인덱스 수정 : 인데스는 수정이 불가능하다. 따라서 삭제 후 다시 생성해야한다.
