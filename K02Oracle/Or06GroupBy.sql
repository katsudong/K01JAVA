...
--select sum(first_name) from employees; 오류발생

/*
count() : 레코드의 개수를 카운트할때 사용하는 함수
*/
...
select 절에 first_name과 같은 단일행은 사용할 수 없다.
*/
...
select department_id,
    count(*),
    trim(to_char(round(avg(salary), 2), '99,000.00'))"평균급여"
from employees
group by department_id 
order by department_id asc;
...
group by절에서 사용한 컬럼은 select 절에서 사용할 수 있으나,
그 외의 단일컬럼은 select절에서 사용할 수 없다.
그룹하된 상태에서 특정 레오크 하나만 선택하는 것이 애매하므로 에러가 발생한다.
*/
/*
...
-- where count(*)>10; /*사원수는 물리적으로 존재하지 않으므로 where절에 쓰면 에러 발생*/
having count(*)>10 /*이와 같은 경우 having절에 조건을 추가해야한다*/
order by department_id asc;
