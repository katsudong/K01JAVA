...
select * from employees where first_name=initcap('john'); --3개인출
...
ltrim(), rtrim() : L[eft]TRIM, R[ight]TRIM
    ...
    ※TRIM은 문자열을 제거할 순 없지만 LTRIM과 RTRIM은 문자열까지 제거할 수 있다.
*/
...
substr() : 문자열에서 시작인덱스부터 길이만큼 잘라서 문자열을 출력한다.
    ...
    참고3] 시작인덱스가 음수면 우측 끝부터 좌로 인덱스를 적용한다.
*/
...
/*
사원테이블의 레코드를 대상으로 문자열 변경을 적용해보자.
102 Lex De Haan LDEHAAN
*/
--102번 사원의 전체정보를 인출한다.
select * from employees where employee_id=102;
--위 함수를 통해 문자열 변경 및 몇가지를 해본다.
select 
...
instr() : 해당 문자열에서 특정문자가 위치한 인덱스값을 반환한다.
    ...
    형식2] instr(컬럼명, '찾을문자', '탐색을 시작할 인덱스', '몇번째문자')
        => 탐색할 인덱스부터 문자를 찾는다. 단, 찾는 문자 중 몇번째에 있는
        문자인지 지정할 수 있다.
*/
--n이 발견된 첫번째 인덱스 반환
select instr('good morning tom', 'n') from dual;
--인덱스 1부터 탐색을 시작해서 n이 발견된 두번째 인덱스 반환
select instr('good morning tom', 'n', 1, 2) from dual; --n이 처음 발견된 9에서 두번째 n이 발견되었으므로 결과값은 11
--인덱스 8부터 탐색을 시작하여 m이 발견된 첫번째 인덱스 반환
