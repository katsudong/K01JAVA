...
형변환함수 / 기타함수
설명 : 데이터타입을 다른 타입으로 변환해야 할 때 사용하는 함수와 기타 함수
**************************/

/*
sysdate : 현재날짜와 시간을 초단위로 반환해준다. 주로 게시판에서 새로운
    포스팅이 있을 때 입력한 날자를 표현하기 위해 사용된다.
*/
...
select
    to_char(sysdate, '오늘은 yyyy년mm월dd일 입니다') "과연 될까?"
from dual; --에러발생
...
/* 시간 포맷 */
--현재시간을 00:00:00 형태로 표기하기
select
    to_char(sysdate, 'HH:MI:SS'),
    to_char(sysdate, 'hh:mi:ss')
from dual;
--현재 날짜와 시간을 한꺼번에 표시하기
select
...
숫자포맷
    0 : 숫자의 자리수를 나타내며 자리수가 맞지 않는 경우
        0으로 자리를 채운다.
    9 : 0과 동일하지만, 자리수가 맞지 않은 경우 공백으로 채운다.
*/
select
    to_char(123, '0000')"서식문자0사용", /*앞에 0이 나와서 이상함*/
    to_char(123, '9999') /*앞에 공백이 있어 이상함*/
from dual;
--숫자에 세자리마다 콤마 표시하기
select
...
/*
날짜포맷이 년-월-일 순이 아닌 경우는 오라클이 인식하지 못하여 에러가 발생된다.
이 떄는 날짜서식을 이용해서 오라클이 인식할 수 있도록 처리해야한다.
*/
...
/*
시나리오] '2020-10-14 15:51:39' 와 같은 형태의 문자열을 날짜로 인식할 수 있도록
    쿼리문을 구성하시오.
*/
--날짜에는 문제가 없으나 시간이 포함되어 에러 발생됨
select
    to_date('2020-10-14 15:51:39')
from dual;
--방법1 : 날짜부분만 잘라와서 인식시킴
select
    substr('2020-10-14 15:51:39', 1, 10) "문자열 자르기",
    to_date(substr('2020-10-14 15:51:39', 1, 10)) "날짜서식으로 변경"
from dual;
...
nvl() : null값을 다른 데이터로 변경하는 함수.
    ...
    NullPointerException이 발생하게 된다. 그러므로 아예 데이터를 가져올 때 null값이 
    나올 수 있는 컬럼에 대해 미리 처리하면 예외발생을 미리 차단할 수 있으므로 편리하다.
*/

select
    first_name, commission_pct, nvl(commission_pct, 0) AS "보너스율",
    (salary+commission_pct) "결과 비정상", (salary+nvl(commission_pct, 0))
from employees;
...
단, 날짜의 형태는 ‘01-01-2020’ 포맷으로 사용한다. 즉 sysdate - ‘01-01-2020’ 
이와같은 연산이 가능해야한다. 
*/
select 
    to_date('01-01-2020', 'dd--mm-yyyy') "날짜서식변경",
    sysdate - to_date('01-01-2020', 'dd--mm-yyyy') "날짜의 차",
