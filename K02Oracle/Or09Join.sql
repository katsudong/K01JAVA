...
--부서테이블에서 가져오기 위해 departments.department_id로 기술함. 정상실행    
select
    employee_id, first_name, last_name, email,
...
--as(알리아스)를 통한 별칭으로 쿼리문을 간소화 할 수 있다.
select
    employee_id, first_name, last_name, email,
...
outer join을 사용할 때는 반드시 outer전에 기준이 되는 테이블을
결정하고 기술해야 한다.
...
형식2(오라클 방식)
    ...
    where
        테이블1.컬럼=테이블2.컬럼(+)
        and 조건1 or 조건2 ... 조건N;
    => 오라클 방식으로 변경시에는 outer join 연산자인 (+)를 붙여주면 된다.
...
/*
    ...
    붙이면 오류가 발생한다.
    using 절에 사용된 컬럼은 좌, 우측의 테이블에 동시에 존재하는 컬럼이라는
...
/*
4. “city : Toronto / state_province : Ontario” 에서 근무하는 모든 사원의 
이름, 업무명, 부서번호 및 부서명을 출력하시오.
출력목록] 사원이름, 업무명, 부서ID, 부서명
*/

select
    job_title, D.department_id, department_name
from locations L
...
모든 담당업무(JOB_ID)의 고유목록(distinct)을 부서의 
도시명(CITY)을 포함하여 출력하시오.
...
출력목록] 이름, 성, 담당업무ID, 급여
*/
--담당업무가 FI_ACCOUNT인 사원들의 매니져 아이디를 조회
select employee_id, first_name, manager_id 
from employees where job_id='FI_ACCOUNT';
--매니져 아이디가 108이므로 사원번호를 조회
select first_name
from employees where employee_id=108;

--셀프조인을 통해서 해당 사원의 매니저 정보를 출력한다.
select 
