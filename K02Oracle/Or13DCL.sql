...
실행해야한다.
사용자 계정 생성 후 접속 테스트는 CMD(명령프롬프트)에서 진행한다.
*/
...
--생성직후 cmd에서 sqlplus 명령으로 접속 시 login denide 에러 발생됨
...
/*
테이블 스페이스란?
    디스크 공간을 소비하는 테이블과 뷰, 그리고 그 밖에 다른 데이터베이스
...
--테이블 생성을 해보면 users 테이블 스페이스에 대한 권한이 없어 생성할 수 없음
...
/*
이미 생성된 사용자의 디폴트 테이블 스페이스를 변경하고 싶은 경우
alter user 명령을 사용한다.
*/
...
sqlplus에서 ED명령어 사용하기
    : 긴 쿼리문을 작성할 때 메모장을 editor로 활용하여 실행시키는 명령
...
--test_user2 계정에서 테이블 생성과 시퀀스 생성을 시도 : 시퀀스는 권한 부족으로 에러 발생됨
