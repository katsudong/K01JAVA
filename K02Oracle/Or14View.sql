...
select employee_id, first_name, last_name, job_id, hire_date, department_id
    from employees where job_id='ST_CLERK';--20개 인출
--뷰를 생성
...
뷰 수정하기
    ...
    따라서 처음 뷰를 생성할 때부터 or replace를 써도 무방하다.
*/
...
/*
    뷰 생성 시 컬럼명을 지정하면 마치 별칭을 준것과 같이 기존
    테이블의 컬럼명을 변경할 수 있다.
*/
select * from view_employees;
...
from employees;
/*
    보너스율이 null인 경우 연봉이 계산되지 않으므로 nvl() 함수를 이용해서
    0으로 값을 변환한 후 계산한다. 또한 to_char()로 세자리마다 컴마를 삽입하고,
    trim()으로 양쪽의 공백을 제거한다.
*/
--step2
