...
설명 : 숫자데이터를 처리하기 위한 숫자관련 함수를 알아보자
    테이블 생성 시 Number 타입으로 선언된 컬럼에 저장된 데이터를 대상으로한다.
*****************/
...
trunc() : 소수점을 특정자리수에서 잘라낼 때 사용하는 함수
    형식 : trunc(컬럼명 혹은 값, 소수점이하자리수)
        두번째 인자가
            ...
            음수일 떄 : 정수부를 숫자만큼 잘라 나머지를 0으로 채움
*/
...
select first_name, last_name, salary, trunc(commission_pct,1), job_id,
...
select first_name, salary, trunc(commission_pct,1) from employees
    where commission_pct is not null;
--3. 별칭으로 처리(영업사원의 job_id는 모두 SA_xxx와 같이 시작한다.)
select first_name, last_name, salary, commission_pct, job_id,
    (salary + (1000 * commission_pct)) AS TotalSalary
    from employees where job_id like 'SA%';
--4. 커미션을 소수점 1자리까지만으로 금액 계산하기
...
round(값, 자리수) : 반올림 처리
    두번째 인자가
        ...
        있는경우 : 숫자만큼 소수점이 표현되므로 그 다음수가 5 이상이면 올림,
            미만이면 버림
*/
...
--첫번째항목 : 소수 이하 6자리까지 표현하므로 7을 올림처리하여 0.123457 출력
--두번째항목 : 소수 이하 4자리까지 표현하므로 1을 버림처리하여 2.3456 출력
...
select mod(99, 4) "99를 4로 나눈 나머지" from dual; --3
select power(2, 10) "2의 10승" from dual; --1024
select sqrt(49) "49의 제곱근" from dual; --7